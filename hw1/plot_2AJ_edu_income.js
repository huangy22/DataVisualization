var Plot_edu_income = function(){
  var outerWidth = 650;
  var outerHeight = 500;
  var margin = { left: 80, top: 60, right: 30, bottom:130 };

  var xColumn = "EduLevel";
  var yColumns = ["Income_mean", "Income_median", "EarnFT_mean", "EarnFT_median", "EarnAll_mean", "EarnAll_median"];

  var innerWidth  = outerWidth  - margin.left - margin.right;
  var innerHeight = outerHeight - margin.top  - margin.bottom;

  var svg = d3.select("#line_edu_income").append("svg")
  ...
  function plot_axis(data){
    ...
    yScale.domain([d3.min(data, function(d){ return d[yColumns[1]];}), d3.max(data, function(d){ return d[yColumns[2]];})]);
    ...
    yAxisG.append("text")
      ...
      .text("Income/Earnings by Education");
  }
  ...
  function render(data, i){
    ...
    dots
      ...
      .attr("cy", function (d){ return yScale(d[yColumns[i]]); })
      .attr("fill", color[i]);

    var line = d3.svg.line()
      .x(function(d) { return xScale(d[xColumn]); })
      .y(function(d) { return yScale(d[yColumns[i]]); }) ;
